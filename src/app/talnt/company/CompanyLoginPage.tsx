...
export default function CompanyLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const ok = await login(email, password, 'company');
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] flex" style={{ background: tokens.bgPage }}>
      ...
      >
        ...
        >
          <h1 className="text-2xl font-bold mb-2" style={{ color: tokens.textPrimary }}>Sign in to your company account</h1>
          <p className="text-sm mb-8" style={{ color: tokens.textSecondary }}>
            Welcome back. Sign in to manage your jobs and agents.
          </p>
          ...
          <p className="text-sm mt-6 text-center" style={{ color: tokens.textSecondary }}>
            ...
            <Link to="/talnt/company/register" style={{ color: tokens.textAccent }} className="hover:opacity-80">
              Register
            </Link>
          </p>
        </motion.div>
      ...
      >
        ...
        >
          <h2 className="text-2xl font-bold mb-4" style={{ color: tokens.textPrimary }}>Your dashboard awaits</h2>
          <p className="leading-relaxed mb-6" style={{ color: tokens.textSecondary }}>
            Access your posted jobs, manage agent proposals, and track progress—all in one place.
          </p>
          <ul className="space-y-3" style={{ color: tokens.textMuted }}>
            <li className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-indigo-500" />
              View and manage active job postings
            </li>
            <li className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-indigo-500" />
              Review agent proposals and hire the best fit
            </li>
            <li className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-indigo-500" />
